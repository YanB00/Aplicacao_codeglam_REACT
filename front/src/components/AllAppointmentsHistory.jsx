...
export default function AllAppointmentsHistory({ userId }) {
  ...
  useEffect(() => {
    const fetchAllAppointments = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`http://localhost:3000/agendamentos/salao/${userId}`);
  ...
  useEffect(() => {
    const fetchFilterData = async () => {
      ...
      setFilterError(null);
      try {
        const servicesResponse = await fetch(`http://localhost:3000/servicos/${userId}`);
        ...
        const employeesResponse = await fetch(`http://localhost:3000/funcionarios/salao/${userId}`);
