...
const AddAppointmentForm = ({ onClose, selectedDate, salaoId, onAppointmentSuccessfullySaved }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                if (!salaoId) {
                    showMessage("ID do Salão não fornecido. Não é possível carregar os dados necessários.", 'error');
                    console.warn("salaoId is missing in AddAppointmentForm, cannot fetch services/clients/employees.");
                    setServices([]);
                    setClients([]);
                    setEmployees([]);
                    setIsLoading(false);
                ...
                if (!servicesRes.ok) {
                    ...
                    showMessage(`Falha ao buscar serviços: ${servicesRes.statusText} - ${errorText}`, 'error');
